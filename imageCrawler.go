...
//		stats.DecrStat("images_to_be_crawled")
		...
		if imageBeenHereUrl(Url) {
			log.V(2).Infof("crawlImages(): We've already been to '%s', skipping!", Url)
//			stats.IncrStat("images_skipped")
		...
		if strings.Contains(strings.ToLower(image.alt), config.SearchString) {
			match = true
			log.V(2).Infof("Match found on ALT tag for URL '%s'!", Url)
		}
		...
		if !match {
			log.V(2).Infof("No match for %s found in alt and title tags for URL '%s', stopping!", Url)
...
func imageBeenHereUrl(url string) bool {
	...
	if len(results) < 5 {
		log.Warningf("imageBeenHereUrl(): Unable to parse URL: '%s'", url)
		return (true)
	}
	host := results[1]
	uri := results[4]
...
func writeImage(Filename string, Body string) {
	...
	if err != nil {
		log.Warningf("Error opening file: %s", err)

	} else {
		n, err := file.Write([]byte(Body))
		if err != nil {
			log.Warningf("Error writing file: '%s': %s", target, err)
		}
